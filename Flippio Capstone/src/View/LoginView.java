...
public class LoginView extends JFrame {
    ...
    private void updateSignupScaling() {
        ...
        // Fix: Set big size AND font for all signup fields and dropdown
        Dimension bigFieldSize = new Dimension(380, 60);
        Font bigFont = FontUtil.montserrat(22f, Font.BOLD, new Font("SansSerif", Font.BOLD, 22));
        if (signNameField != null) {
            signNameField.setPreferredSize(bigFieldSize);
            signNameField.setMaximumSize(bigFieldSize);
            signNameField.setFont(bigFont);
        }
        if (signIdField != null) {
            signIdField.setPreferredSize(bigFieldSize);
            signIdField.setMaximumSize(bigFieldSize);
            signIdField.setFont(bigFont);
        }
        if (signRoleCombo != null) {
            signRoleCombo.setPreferredSize(bigFieldSize);
            signRoleCombo.setMaximumSize(bigFieldSize);
            signRoleCombo.setFont(bigFont);
        }
        if (signPassField != null) {
            signPassField.setPreferredSize(bigFieldSize);
            signPassField.setMaximumSize(bigFieldSize);
            signPassField.setFont(bigFont);
        }
        if (signConfirmPassField != null) {
            signConfirmPassField.setPreferredSize(bigFieldSize);
            signConfirmPassField.setMaximumSize(bigFieldSize);
            signConfirmPassField.setFont(bigFont);
        }
    ...
    private void addPasswordToggle(JPasswordField pf) {
        ...
        // Load icon from classpath first, then fall back to common filesystem locations
        java.net.URL iconUrl = getClass().getResource("/Resources/graduation.png");
        ImageIcon rawIcon = null;
        if (iconUrl != null) {
            rawIcon = new ImageIcon(iconUrl);
        } else {
            try {
                java.nio.file.Path p = java.nio.file.Paths.get("Resources", "graduation.png");
                if (java.nio.file.Files.exists(p)) {
                    rawIcon = new ImageIcon(p.toUri().toURL());
                } else {
                    java.nio.file.Path p2 = java.nio.file.Paths.get("src", "Resources", "graduation.png");
                    if (java.nio.file.Files.exists(p2)) {
                        rawIcon = new ImageIcon(p2.toUri().toURL());
                    }
                }
            } catch (Exception ex) {
                // ignore and fall back to text icon
            }
        }
        final ImageIcon showIcon = (rawIcon != null)
            ? new ImageIcon(getScaledImage(rawIcon.getImage(), iconSize, iconSize))
            : null;
        final ImageIcon hideIcon = showIcon; // Use same icon for both states, or load a different one if available
    ...
    private JButton createBackButton() {
        ...
        btn.addActionListener(e -> {
            // Clear any entered data in both login and signup forms before returning
            resetAllInputFields();
    ...
    // Clear all input fields in login and signup panels
    private void resetAllInputFields() {
        try {
            ...
            if (signPassField != null) {
                ...
                signPassField.setEchoChar('•');
            }
            if (signConfirmPassField != null) {
                signConfirmPassField.setText("");
                signConfirmPassField.setEchoChar('•');
            }
            if (signRoleCombo != null && signRoleCombo.getItemCount() > 0) {
                signRoleCombo.setSelectedIndex(0);
            }
        } catch (Exception ex) {
            // Defensive: ignore any runtime exception while clearing fields
        }
    }

    // High-quality image scaler to avoid blurry icons when resizing
    private Image getScaledImage(Image src, int w, int h) {
        if (src == null) return null;
        int srcW = src.getWidth(null);
        int srcH = src.getHeight(null);
        if (srcW <= 0 || srcH <= 0) {
            // fallback to straightforward scaling
            return src.getScaledInstance(w, h, Image.SCALE_SMOOTH);
        }

        BufferedImage srcBi = new BufferedImage(srcW, srcH, BufferedImage.TYPE_INT_ARGB);
        Graphics2D gSrc = srcBi.createGraphics();
        gSrc.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        ...
        g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        g2.drawImage(srcBi, 0, 0, w, h, null);
        g2.dispose();
        return dest;
    }
